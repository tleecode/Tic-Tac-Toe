...
def result(board, action):
    ...
    """
    if action not in actions(board): #check if action is valid action
        raise Exception("Invalid Action")

    row, col = action
    board_copy = copy.deepcopy(board)
    board_copy[row][col] = player(board) #check who is the next player
    return board_copy
...
def terminal(board):
    ...
    for row in range(len(board)):
        for col in range(len(board[row])):
            if board[row][col] == EMPTY: #game is still being played, so no winner
                return False
